...
reset_database(){
 # Refresh database from production
    ...
    drush si minimal -y --account-name=dev --account-pass=dev --db-url=mysql://${db_username}:${db_password}@localhost/${db_name} --site-name=${site_name}
